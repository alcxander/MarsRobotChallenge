export class MarsRobot {
    ...
    collectSampleAtCurrentPosition() {
        // const currentTerrain = this.terrain[this.position.y][this.position.x]
...
export function simulateRobot(input) {
    ...
    const validTerrains = ["Fe", "Se", "W", "Si", "Zn", "Obs"];
    //validate terrain types
    function isValidTerrain(t) {
        const valid = validTerrains.includes(t);
        if (!valid) {
